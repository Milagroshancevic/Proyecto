...
// detalles aire
  ...
  .then(function(datosA){
    ...
    for(i=0;i<aire.length;i++){
      detalleA.innerHTML+= '<p>' + aire[i].name + '</p>' + '<p>'+ "Genero: " +  aire[i].genre_ids + '</p>' + '<p>' + "Lenguaje: " + aire[i].original_language + '</p>' + '<p>' + aire[i].overview + '</p>'+ '<p>' + aire[i].first_air_date + '</p>'  + '<img src="' + "https://image.tmdb.org/t/p/original/" + aire[i].poster_path +'" >' + '<br>'+ '<button id="button-reco"type="button" name="recom">'+ "Ver recomendaciones" + '</button>'
      }
})
